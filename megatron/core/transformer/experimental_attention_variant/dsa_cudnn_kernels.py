...
def _indexer_topk_packed_thd(
    ...
    if cp_size == 1:
        ...
        segment_offsets -= torch.repeat_interleave(
            score_cu_k[:-1].to(dtype=torch.int64), segment_k_lengths, output_size=total_segment_k
        )
        source_indices = segment_key_starts.index_select(0, segment_ids) + segment_offsets
        score_q = q_bshd[0]
        score_k = k_bshd[0].index_select(0, source_indices).contiguous()
        score_w = w_bsh[0]
